...
allele.qc <- function(a1, a2, ref1, ref2) {
  ...
  # Create complements of reference alleles
  flip1 <- chartr("ATGC", "TACG", ref1)
  flip2 <- chartr("ATGC", "TACG", ref2)
  # Flip if alleles are swapped OR strand-complement swapped
  flip <- (a1 == ref2 & a2 == ref1) | (a1 == flip2 & a2 == flip1)
  
  # Only keep valid single-base alleles
  valid <- a1 %in% c("A", "C", "G", "T") & a2 %in% c("A", "C", "G", "T")
  # Remove strand-ambiguous SNPs (A/T or C/G)
  ambig <- (a1 == "A" & a2 == "T") | (a1 == "T" & a2 == "A") |
    (a1 == "C" & a2 == "G") | (a1 == "G" & a2 == "C")
  keep <- valid & !ambig
  
  return(list(keep = keep, flip = flip))
}
...
sumstat <- fread(
  file = opt$sumstats, 
  header = TRUE, 
  select = c("SNP", "A1", "A2", "Z")
)

# Load in list of weights
# TODO : TEST FOR NO HEADER HERE
wgtlist <- read.table(opt$weights, head = TRUE, as.is = TRUE)
...
out.tbl <- data.frame(
  ...
  "P1" = character(N), 
  "HSQ" = numeric(N), 
  "BEST.GWAS.ID" = character(N), 
  "BEST.GWAS.Z" = numeric(N), 
  "EQTL.ID" = character(N), 
  "EQTL.R2" = numeric(N), 
  "EQTL.Z" = numeric(N), 
  "EQTL.GWAS.Z" = numeric(N), 
  "NSNP" = numeric(N), 
  "NWGT" = numeric(N), 
  "MODEL" = character(N), 
  "MODELCV.R2" = character(N), 
  "MODELCV.PV" = character(N), 
  "TWAS.Z" = numeric(N), 
  "TWAS.P" = numeric(N), 
...
if (!is.na(opt$coloc_P)) {
  ...
  if (sum(names(wgtlist) == "N") == 0) {
    if (sum(names(wgtlist) == "PANEL") == 0 || is.na(opt$PANELN)) {
      ...
      paneln <- read.table(opt$PANELN, as.is = TRUE, head = TRUE, sep = "\t")
...
raw_sample_ct <- length(readLines(paste(opt$ref_ld_chr, chr, ".fam", sep = "")))
pvar <- pgenlibr::NewPvar(paste0(opt$ref_ld_chr, chr, ".bim"))
pgen <- pgenlibr::NewPgen(
  filename = paste(opt$ref_ld_chr, chr, ".bed", sep = ""), 
  pvar = pvar, 
  raw_sample_ct = raw_sample_ct
)
variant_ids <- vapply(
  seq_len(GetVariantCt(pvar)),
  function(i) GetVariantId(pvar, i),
...
for (w in 1:nrow(wgtlist)) {
  ...
  # Find where the variants in the weight file 
  # located in the reference file
  idx_pvar <- match(snps[, 2], variant_ids)
  # If there isn't a matched variant in the reference, drop it
  keep <- !is.na(idx_pvar)
  matched_variant_ids <- snps[keep, 2]
  # Find where the variants that's in both the weight file and reference
  # located in the summary statistics file
  idx_sumstat <- match(matched_variant_ids, sumstat$SNP)
  # Subset the summary statistics to rows that has `matched_variant_ids`
  # If it doesn't have it, fill the row with NA
  sumstat_subset <- sumstat[idx_sumstat, , drop = FALSE]
  sumstat_missing_snps <- is.na(sumstat_subset$SNP)
  sumstat_subset$SNP <- matched_variant_ids
  
  # GetVariantsById(pvar, id)
  
  # For those variants missing in summary statistics
  # Fill in A1 and A2 from the reference
  if (any(sumstat_missing_snps)) {
    idx <- idx_pvar[keep][sumstat_missing_snps]
    sumstat_subset$A1[sumstat_missing_snps] <- vapply(
      idx,
      function(i) GetAlleleCode(pvar, i, 2),
      FUN.VALUE = character(1)
    )
    sumstat_subset$A2[sumstat_missing_snps] <- vapply(
      idx,
      function(i) GetAlleleCode(pvar, i, 1),
      FUN.VALUE = character(1)
    )
  }
  
  # Two steps of QC here
  ## QC 1: allele-flip the input and output
  ref_A1 <- vapply(idx_pvar[keep], function(i) GetAlleleCode(pvar, i, 2), FUN.VALUE = character(1))
  ref_A2 <- vapply(idx_pvar[keep], function(i) GetAlleleCode(pvar, i, 1), FUN.VALUE = character(1))
  qc <- allele.qc(sumstat_subset$A1, sumstat_subset$A2, ref_A1, ref_A2)
  ## Flip Z-scores for mismatching alleles
  sumstat_subset$Z[qc$flip] <- -1 * sumstat_subset$Z[qc$flip]
  sumstat_subset$A1[qc$flip] <- ref_A1[qc$flip]
  sumstat_subset$A2[qc$flip] <- ref_A2[qc$flip]
  # QC 2
  ## qc$keep
  snps <- snps[qc$keep, ]
  wgt.matrix <- wgt.matrix[qc$keep, , drop = FALSE]
  cur.genos <-scale(ReadList(pgen, idx_pvar[keep][qc$keep], meanimpute = TRUE))
  cur.Z <- sumstat_subset$Z[qc$keep]
  # Flip WEIGHTS for mismatching alleles
  qc <- allele.qc(snps[, 5], snps[, 6], ref_A1[qc$keep], ref_A2[qc$keep])
  ...
  if (length(cur.Z) == 0) {
    ...
    cur.LD <- crossprod(cur.genos) / (nrow(cur.genos) - 1)
